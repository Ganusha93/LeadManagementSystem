...
public class CustomerDetailsDAO {
    ...
    public static void insertCutomerDetails(CustomerDetailsDTO cddto, Connection connection) {
        PreparedStatement pst = null;
        String sql = null;
        int result = 0;

        try {
            sql = "INSERT INTO LMS_CUSTOMER_DETAILS (CUS_NAME,USER_ID,GENDER,MOB_NUMBER,LEAD_SRC,SAL_ACT_STAGE) VALUES "
                    + "('" + cddto.getCusName() + "','" + cddto.getUserID() + "','" + cddto.getGender() + "','" + cddto.getMobNum() + "',"
                    + "'" + cddto.getLeadSource() + "','" + cddto.getSalActStage() + "')";
    ...
    public List searchCustomerDetails(String user, List list, Connection connection) {
        ...
        try {
            ...
            if (list.get(0) != "") {
                s = s + " AND CUS_NAME=" + "'" + list.get(0) + "'";
            }
            if (list.get(1) != "") {
                s = s + " AND STATUS=" + "'" + list.get(1) + "'";
            }
            if (list.get(2) != "") {
                s = s + " AND SAL_ACT_STAGE=" + "'" + list.get(2) + "'";
            }
            if (list.get(3) != "") {
                s = s + " AND LEAD_SRC=" + "'" + list.get(3) + "'";
            }
            if (list.get(4) != "") {
                s = s + " AND POL_STATUS=" + "'" + list.get(4) + "'";
            }
            if (list.get(7) != "") {
                s = s + " AND NIC=" + "'" + list.get(7) + "'";
            }
